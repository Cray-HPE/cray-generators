...
class Git {
  ...
  openPullRequest (repoUrl, branchName, options = {}) {
    ...
    return new Promise ((resolve, reject) => {
      const sourceUrlParts  = this.parseRepoUrlForApi(repoUrl)
      const destUrlParts    = this.parseRepoUrlForApi(options.destinationRepoUrl)
      ...
      request(requestOptions, (error, response, body) => {
        ...
        if (response && response.statusCode != 201) {
          return reject(`Received response code ${response.statusCode} from ${uri} ` +
  ...
  fork (repoUrl, destinationProject) {
    return new Promise((resolve, reject) => {
      const urlParts        = this.parseRepoUrlForApi(repoUrl)

...
class Git {
  ...
  commitAndPush (localClonePath, commitMessage, force = false) {
    return this.shell.exec('git', ['status', '-s'], { cwd: localClonePath }).then((result) => {
      return result.stdout.trim() == '' ? false : this.shell.exec('git', ['add', '.'], { cwd: localClonePath })
    }).then((result) => {
      return result === false ? false : this.shell.exec('git', ['commit', '-m', commitMessage], { cwd: localClonePath })
    }).then((result) => {
      return result === false ? false : this.shell.exec('git', ['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: localClonePath })
    }).then((result) => {
      if (result !== false) {

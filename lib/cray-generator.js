...
class CrayGenerator extends Generator {
   ...
   * @returns {Promise} no result from the successful promise
   */
  _writeTemplate (baseFilePath) {
    const write = () => {
      this.fs.copyTpl(
        this.templatePath(`${baseFilePath}.tpl`),
        this.destinationPath(baseFilePath),
        this.props
      )
      return Promise.resolve()
    }
    if (this.fse.existsSync(this.destinationPath(baseFilePath)) && falsey(this.options.overwrite)) {
      return this.prompt([ {
        type: 'confirm',
        name: 'proceed',
        message: `${baseFilePath} already exists in your repo, would you like to overwrite?`,
        default: false,
      }]).then(() => {
        return write()
      })

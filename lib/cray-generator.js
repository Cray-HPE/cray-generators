...
class CrayGenerator extends Generator {
   ...
   * @param {string} [templatePath=null] a specific template path to use, by default uses the corresponding path determined from baseFilePath
   * @param {string} [variables=null] a specific set of variables to pass in to the template rendering engine
   */
  _writeTemplate (baseFilePath, destinationPath = null, templatePath = null, variables = null) {
    if (destinationPath === null) {
      destinationPath = this.destinationPath(baseFilePath)
    }
    this.fs.copyTpl(
      this.templatePath(`${(templatePath || baseFilePath)}.tpl`),
      destinationPath,
